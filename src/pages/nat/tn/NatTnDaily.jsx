...
export default function NatTnDaily() {
  ...
  const getDataSelect = async () => {
    try {
      const resultSelect = await axios.get(`${BASE_URL}/nat/tn/tn-daily/select`);
  ...
  const getData = async () => {
    try {
      const resultData = await axios.post(`${BASE_URL}/nat/tn/tn-daily/data`, {
  ...
  return (
    <div className="grid gap-4">
      <PageBreadcrumb pageTitle={`NAT : TURNING DAILY`} />
