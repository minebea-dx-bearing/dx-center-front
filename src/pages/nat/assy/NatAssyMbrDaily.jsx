...
export default function NatAssyMbrDaily() {
  ...
  const getDataSelect = async () => {
    try {
      const resultSelect = await axios.get(`${BASE_URL}/nat/assy/mbr-daily/select`);
  ...
  const getData = async () => {
    try {
      const resultData = await axios.post(`${BASE_URL}/nat/assy/mbr-daily/data`, {
  ...
  return (
    <div className="grid gap-4">
      <PageBreadcrumb pageTitle={`NAT : ASSY MBR DAILY`} />
