...
export default function McbAssyAnalysisMachine({ defaultMC, defaultDate }) {
  ...
  const GET_MASTER_MACHINE_MBR_API = async () => {
    try {
      let dataMC = await axios.get(
        `${LOCAL_URL}/mcb/assy/analisis-by-mc/master_machine_ap`
      );
  ...
  const GET_PRODUCTION_BY_HOUR_API = async () => {
    try {
      ...
      let data = await axios.get(
        `${LOCAL_URL}/mcb/assy/analisis-by-mc/ap_production_hour_by_mc/${machine}/${date.format(
          "YYYY-MM-DD"
        )}`
      );
  ...
  const GET_STATUS_BY_HOUR_API = async () => {
    try {
      setLoading(true);
      let data = await axios.get(
        `${LOCAL_URL}/mcb/assy/analisis-by-mc/status_ap/${machine}/${date.format(
          "YYYY-MM-DD"
        )}`
      );
  ...
  const GET_PRODUCTION_SHIFT_API = async () => {
    try {
      setLoading(true);
      let data = await axios.get(
        `${LOCAL_URL}/mcb/assy/analisis-by-mc/get_production_analysis_by_mc/${machine}/${date.format(
          "YYYY-MM-DD"
        )}`
      );
